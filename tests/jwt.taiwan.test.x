...
describe('Taiwan JWT Verifier', () => {
  ...
  beforeAll(async () => {
    ...
    circuit = await wasm_tester(path.join(__dirname, './test-circuits/jwt-taiwan-test.circom'), {
      recompile: RECOMPILE,
  ...
  it('should verify taiwan JWT', async () => {
    ...
    console.log("Taiwan VC JWT Circuit verified");
  });
});
