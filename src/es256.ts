...
function get_x_y_from_pk(pk: string) {
  ...
  const bitString = reader.readString(3, true); // 3 = BIT STRING, true = return raw buffer
  if (!bitString || !(bitString instanceof Buffer)) {
    throw new Error("Failed to read EC public key BIT STRING.");
  }

  const xy = bitString.subarray(2); // skip the 2-byte prefix (0x00 + 0x04)
...
export function generateES256Inputs(params: Es256CircuitParams, message: Buffer, signature: string, pk: string) {
  ...
  let pk1 = pk.replace("-----BEGIN PUBLIC KEY-----", "").replace("-----END PUBLIC KEY-----", "").replace(/\s+/g, "");
  let [x, y] = get_x_y_from_pk(pk1);
  let pubkey = new p256.ProjectivePoint(bufferToBigInt(x), bufferToBigInt(y), 1n);
  ...
  return {
    ...
    message: Uint8ArrayToCharArray(messagePadded).map((char) => parseInt(char)),
